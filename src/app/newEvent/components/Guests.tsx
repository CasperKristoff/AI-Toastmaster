...
const Guests: React.FC<GuestsProps> = ({
  ...
  const handleDeleteGuest = (guestId: string) => {
    const updatedEvent: Event = {
      ...event,
      guests: event.guests.filter((guest: Guest) => guest.id !== guestId),
    };
      ...
      {event.guests.length === 0 ? (
        ...
        <div className="space-y-3 w-full">
          {event.guests.map((guest: Guest) => (
            <div
              key={guest.id}
              className="group bg-white/80 backdrop-blur-xl rounded-2xl p-4 border border-dark-royalty/10 hover:border-dark-royalty/30 transition-all duration-300 hover:shadow-lg hover:bg-white/90 w-full relative"
            >
              <div className="flex items-center space-x-4 w-full">
                ...
                <div className="flex-1 min-w-0">
                  {editingGuest === guest.id ? (
                    ...
                    <>
                      <h4 className="text-lg font-bold text-dark-royalty group-hover:text-deep-sea transition-colors truncate">
                        {guest.name}
                      </h4>
                    </>
                  )}
                  ...
                  <div className="relative flex-shrink-0">
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        setOpenMenuId(
                          openMenuId === guest.id ? null : guest.id,
                        );
                      }}
                    ...
                    {/* Dropdown Menu */}
                    {openMenuId === guest.id && (
                      <div className="absolute right-0 top-full mt-1 bg-white/95 backdrop-blur-xl rounded-xl border border-dark-royalty/20 shadow-lg z-50 min-w-[120px]">
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            setOpenMenuId(null);
