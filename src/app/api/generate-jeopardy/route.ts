      ...
      if (parsedResponse.categories && Array.isArray(parsedResponse.categories)) {
        ...
        if (parsedResponse.categories.length !== boardSize) {
          return NextResponse.json({
            ...
            suggestions: 'Try regenerating the categories to get the correct number.'
          });
        }
        ...
        for (let i = 0; i < parsedResponse.categories.length; i++) {
          ...
          if (!category.questions || category.questions.length !== boardSize) {
            return NextResponse.json({
              ...
              suggestions: 'Try regenerating the categories to get the correct number of questions per category.'
            });
          }
        }
