...
export interface ImageService {
  uploadImage: (file: File, userId: string, eventId: string, onProgress?: (progress: number) => void) => Promise<string>;
  uploadMultipleImages: (files: File[], userId: string, eventId: string, onProgress?: (progress: number) => void) => Promise<string[]>;
...
export const imageService: ImageService = {
  ...
  async uploadImage(file: File, userId: string, eventId: string, onProgress?: (progress: number) => void): Promise<string> {
    try {
  ...
  async uploadMultipleImages(files: File[], userId: string, eventId: string, onProgress?: (progress: number) => void): Promise<string[]> {
    try {
      ...
      for (const file of files) {
        try {
          console.log(`Uploading file ${completedUploads + 1}/${files.length}: ${file.name}`);
          const downloadURL = await this.uploadImage(
            ...
            (progress) => {
              ...
              const overallProgress = ((completedUploads + progress / 100) / files.length) * 100;
      ...
      console.log(`Upload completed: ${downloadURLs.length}/${files.length} images uploaded successfully`);
      return downloadURLs;
    } catch (error) {
